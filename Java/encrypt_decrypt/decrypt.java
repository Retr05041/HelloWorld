...
This decrypt program decrypts an encrypted string using a password
Usage: 'java decrypt <word> <password>'
*/

public class decrypt {

    public static String decryptString(String message, String password) {
        int m = 0, p = 0, i;
        ...
        for (i = 0; i < len; i++)
        {
            messageChar[m] = (char)(65 + (50 + (messageChar[m] & 0x9F) - (passwordChar[p] & 0x9F)) % 26); // Decrypts each char in the word
            m++;
            p++;
            if (passwordChar[p] == 0)
                p = 0;
    ...
    public static void main(String[] args) {
        if (args.length < 2) { // I/O Check
            System.out.println("Usage: java decrypt <word> <password>");
        ...
        // Decrypt the word
        String decrypted = decryptString(encrypted, args[1]);
        // Output the word
